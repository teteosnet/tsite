...
if [[ ! $1 = "install" ]]; then
    if [[ ! -d /etc/nginx ]]; then
        echo " [!] Nginx not installed! Aborting..."
      if [[ $0 = "/teteosnet/Bin/tsite" || $0 = "/usr/sbin/tsite" ]]; then
            echo " [i] Type \"tsite install\" for start installation by tsite."
      else
            echo " [i] Type \"$0 install\" for start installation by tsite."
      fi
      exit 1
    fi
fi
...
checkdomain(){
      ...
      elif [[ $1 = "default" || $1 = "000-default" || $1 = "000-default-tsite" ]]; then
...
createsitefile(){
      if ! [[ $1 = "" ]]; then
            ...
            echo "server { # File generated by tsite tool. ( github:teteosnet/tsite )" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    # Listen ports" >> /etc/nginx/sites-available/$1
            echo "    listen 80;" >> /etc/nginx/sites-available/$1
            if [[ $1 = "000-default-tsite" || -f "/etc/nginx/snippets/self-signed.conf" || -f "/etc/nginx/snippets/ssl-params.conf" ]]; then
            echo "    listen 443 ssl;" >> /etc/nginx/sites-available/$1
            echo "    include snippets/self-signed.conf;" >> /etc/nginx/sites-available/$1
            echo "    include snippets/ssl-params.conf;" >> /etc/nginx/sites-available/$1
            fi
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    # Domain" >> /etc/nginx/sites-available/$1
            echo "    set \$domain \"/var/www/$1/public_html/\";" >> /etc/nginx/sites-available/$1
            if ! [[ $1 = "000-default-tsite" || $1 = "000-default" || $1 = "default" ]]; then
            echo "    server_name $1 *.$1;" >> /etc/nginx/sites-available/$1
            else
            echo "    server_name localhost *.localhost $1 *.$1;" >> /etc/nginx/sites-available/$1
            fi
            echo "    server_tokens off;" >> /etc/nginx/sites-available/$1
            echo "    root \$domain;" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    # Include .nginx.conf from www folder" >> /etc/nginx/sites-available/$1
            echo "    include /var/www/$1/public_html/.nginx[.]conf;" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    # Index with PHP 8.2" >> /etc/nginx/sites-available/$1
            echo "    index index.html index.htm index.php;" >> /etc/nginx/sites-available/$1
            echo "    location ~ \.php$ {" >> /etc/nginx/sites-available/$1
            echo "        include snippets/fastcgi-php.conf;" >> /etc/nginx/sites-available/$1
            echo "        fastcgi_pass unix:/var/run/php/php8.2-fpm.sock;" >> /etc/nginx/sites-available/$1
            echo "     }" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    #" >> /etc/nginx/sites-available/$1
            echo "    # Security" >> /etc/nginx/sites-available/$1
            echo "    #" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    # Block access files start with .ht (for protect .htaccess .htpassword etc...)" >> /etc/nginx/sites-available/$1
            echo "    location ~ /\.ht {" >> /etc/nginx/sites-available/$1
            echo "        deny all;" >> /etc/nginx/sites-available/$1
            echo "    }" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    add_header X-Frame-Options SAMEORIGIN;" >> /etc/nginx/sites-available/$1
            echo "    add_header X-XSS-Protection \"1; mode=block\";" >> /etc/nginx/sites-available/$1
            echo "    #add_header Content-Security-Policy \"default-src 'self';\" always;" >> /etc/nginx/sites-available/$1
            echo "    add_header X-Content-Type-Options \"nosniff\";" >> /etc/nginx/sites-available/$1
            echo "    " >> /etc/nginx/sites-available/$1
            echo "    # ssl-tls" >> /etc/nginx/sites-available/$1
            echo "    #add_header Strict-Transport-Security \"max-age=31536000; includeSubDomains\";" >> /etc/nginx/sites-available/$1
            echo "}" >> /etc/nginx/sites-available/$1
...
if [[ $1 = "" || $1 = "--help" || $1 = "-help" || $1 = "help" || $1 = "/?" || $1 = "?" ]]; then
      ...
      echo ""
      echo " - File Manager: "
      echo "      www [domain name]              open /var/www/[domain name] folder"
      echo "      cd [domain name]               change dir to /var/www/[domain name] folder"
      echo "      phpini                         open php.ini (8.2) file with text editor"
      echo ""
      exit 0
...
# ========================================================================
      ...
      # Check system is Ubuntu 22.04 or PiluX v1 right?
      lsbrelease=`cat /etc/lsb-release | grep "DISTRIB_ID=" | sed "s/DISTRIB_ID=//g"`
      if ! [[ $lsbrelease = "Ubuntu" || $lsbrelease = "PiluX" || $lsbrelease = "PiluX Server" && -d "/etc/apt" ]]; then
      echo " [!] Your OS is not supported. Only PiluX and Ubuntu supported!"
      echo " [i] We planned may be support more OS. Please check Github teteosnet/tsite for updates."
      exit 1
      fi

      # Warn user and wait 5 seconds.
      echo ""
      echo " ------------------------------------------------------------------"
      echo " Nginx and PHP8.2 starting to install!"
      echo " "
      echo " Warning, script deleting /var/www/ and /etc/nginx/sites-*/."
      echo " By continue to installation OWN RISK!"
      echo ""
      ...
      echo " [i] Adding Nginx repo... (ondrej/nginx)"
      add-apt-repository ppa:ondrej/nginx -y &>/dev/null
      echo " [i] Adding PHP repo... (ondrej/php)"
      ...
      chmod -R 774 /var/www
      if [[ ! -d "/etc/nginx" ]]; then
            echo " [!] Nginx cannot installed! Aborting..."
            exit 1
      fi
      echo " [i] Installing PHP8.2..."
      apt install -y php8.2-fpm php8.2-cli php-mysql &>/dev/null
      echo " [i] Installing PHP Extensions..."
      echo " - xml (dom)"
      apt install -y php-xml &>/dev/null
      echo " - json"
      apt install -y php-json &>/dev/null
      echo " - curl"
      ...
      echo " - gmp"
      apt install -y php-gmp &>/dev/null
      sed -i "s/expose_php.*/expose_php = off/g" /etc/php/8.2/cli/php.ini &>/dev/null
      echo " [i] Installing MySQL Server..."
      ...
      php -r "if (hash_file('sha384', 'composer-setup.php') === '55ce33d7678c5a611085589f1f3ddf8b3c52d662cd01d4ba75c0ee0459970c2200a51f492d557530c71c15d8dba01eae') { echo 'Installer verified'; } else { echo 'Installer corrupt'; unlink('composer-setup.php'); } echo PHP_EOL;" &>/dev/null
      ...
      echo " - install python3-acme from apt"   
      apt install -y python3-acme &>/dev/null
      echo " - clone certbot/certbot"
      git clone https://github.com/certbot/certbot &>/dev/null
      echo " - setup.py for acme"
      mv certbot certbot-cloned && cd certbot-cloned/acme
      python3 setup.py install &>/dev/null
      echo " - setup.py for certbot"
      cd ../certbot/
      python3 setup.py install &>/dev/null
      echo " - setup.py for certbot-nginx"
      cd ../certbot-nginx
      python3 setup.py install &>/dev/null
      echo " - setup.py for cloudflare, google, digitalocean"
      cd ../certbot-dns-cloudflare
      python3 setup.py install &>/dev/null
      cd ../certbot-dns-google
      python3 setup.py install &>/dev/null
      cd ../certbot-dns-digitalocean
      python3 setup.py install &>/dev/null
      cd ../../
      rm -rf certbot-cloned
      echo " [i] Configuring Nginx"
      ufw allow 'Nginx HTTP' &>/dev/null
      echo "systemctl restart nginx.service">/root/restart_nginx.sh &>/dev/null
      echo "systemctl status nginx.service">>/root/restart_nginx.sh &>/dev/null
      chmod +x /root/restart_nginx.sh &>/dev/null
      systemctl stop nginx.service &>/dev/null
      rm -rf /etc/nginx/sites-enabled/* && rm -rf /etc/nginx/sites-available/*
      createsitefile "000-default-tsite"
      chmod 644 /etc/nginx/sites-available/000-default-tsite && ln -s /etc/nginx/sites-available/000-default-tsite "/etc/nginx/sites-enabled/000-default-tsite"
      rm -rf /var/www/* && mkdir -p /var/www/000-default-tsite/public_html/
      unlink $HOME/www &>/dev/null
      ln -s /var/www/ $HOME/www
      echo "<html>" > /var/www/000-default-tsite/public_html/index.php
      echo "  <head>" >> /var/www/000-default-tsite/public_html/index.php
      echo "    <title>Nginx Server with PiluX OS</title>" >> /var/www/000-default-tsite/public_html/index.php
      echo "  </head>" >> /var/www/000-default-tsite/public_html/index.php
      echo "  <body style=\"font-family: Arial; background:black; color: white;\">" >> /var/www/000-default-tsite/public_html/index.php
      echo "    <p style=\"text-align:center\"><span style=\"font-size:28px\"><br><br>Congratulations! Your website is working!</span></p>" >> /var/www/000-default-tsite/public_html/index.php
      echo "    <p style=\"text-align:center\"><span style=\"color:#8e44ad\"><span style=\"font-size:20px\">PiluX 💕 Nginx </span></span></p>" >> /var/www/000-default-tsite/public_html/index.php
      echo "  </body>" >> /var/www/000-default-tsite/public_html/index.php
      echo "</html>" >> /var/www/000-default-tsite/public_html/index.php
      if nginx -t > /dev/null 2>&1; then
            systemctl restart nginx.service &>/dev/null
      else
            echo " [!] Warning: Restart canceled. Nginx test failed."
            echo " [i] Type \"nginx -t\" to see error."
      fi
...
# ========================================================================
      ...
      if [[ ! $XDG_CURRENT_DESKTOP = "" ]]; then
            if [[ -f "/teteosnet/OS/IsPiluX" && `cat /teteosnet/OS/IsPiluX` == "true" ]]; then
                  sudo -H xdg-open /var/www/$2 &>/dev/null
            else
                  xdg-open /var/www/$2 &>/dev/null
            fi
            exit 0
      else
            random_string=$(tr -dc A-Za-z0-9 </dev/urandom | head -c 8 ; echo '')
            mkdir -p /teteosnet/Temp/tsite_$random_string/
            echo "cd /var/www/$2">/teteosnet/Temp/tsite_$random_string/cd.sh
            echo "\$SHELL">>/teteosnet/Temp/tsite_$random_string/cd.sh
            chmod +x /teteosnet/Temp/tsite_$random_string/cd.sh
            exec /teteosnet/Temp/tsite_$random_string/cd.sh
            unset randomstring
            rm -rf /teteosnet/Temp/tsite_*
            exit 0
      fi
...
# - phpini
# ========================================================================
elif [[ $1 = "phpini" || $1 = "php.ini" ]]; then

      if [[ -f "/etc/php/8.2/cli/php.ini" ]]; then
            if [[ ! $XDG_CURRENT_DESKTOP = "" ]]; then
                  if [[ -f "/teteosnet/OS/IsPiluX" && `cat /teteosnet/OS/IsPiluX` == "true" ]]; then
                        sudo -H xdg-open /etc/php/8.2/cli/php.ini &>/dev/null
                  else
                        xdg-open /etc/php/8.2/cli/php.ini &>/dev/null
                  fi
                  exit 0
            else
            ...
            echo " [!] /etc/php/8.2/cli/php.ini file not found!"
            exit 1
      fi
...
# ========================================================================
      ...
      SiteCount=`ls /etc/nginx/sites-available | sed -z 's/000-default-tsite\n//g' | sed -z 's/000-default\n//g' | sed -z 's/default\n//g' | wc -l`
      if (( $SiteCount > 0 )); then
            ls /etc/nginx/sites-available | sed -z 's/000-default-tsite\n//g' | sed -z 's/000-default\n//g' | sed -z 's/default\n//g'
      else
...
# ========================================================================
      ...
      # === === Add site now!
            ...
            # Create site file
            if [ -f "/etc/nginx/sites-available/000-default-tsite" ]; then
                  cp /etc/nginx/sites-available/000-default-tsite /etc/nginx/sites-available/$2 &>/dev/null
                  sed -i "s/000-default-tsite/$2/g" /etc/nginx/sites-available/$2 &>/dev/null
                  sed -i "s/localhost//g" /etc/nginx/sites-available/$2 &>/dev/null
            ...
            if [ -d "/var/www/$2/public_html" ]; then
                  ...
                  if [ -f "/var/www/000-default-tsite/public_html/index.php" ]; then
                        cp /var/www/000-default-tsite/public_html/index.php /var/www/$2/public_html/index.php &>/dev/null
                  elif [ -f "/var/www/000-default/public_html/index.php" ]; then
...
# ========================================================================
      ...
      if [[ -f "/etc/nginx/sites-available/000-default-tsite" ]]; then
            if ! grep -q "listen 443 ssl" /etc/nginx/sites-available/000-default-tsite
            then
                  sed -i "s/server {/server {\n    listen 443 ssl;\n    include snippets\/self-signed.conf;\n    include snippets\/ssl-params.conf;/g" /etc/nginx/sites-available/000-default-tsite
            fi
      fi
      echo " [i] Setting Firewall rules..."
      ufw allow 'Nginx Full' &>/dev/null
